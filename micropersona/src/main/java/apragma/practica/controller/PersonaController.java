...
public class PersonaController {
	...
	@ApiResponses(value = { @ApiResponse(code = 200, message = "Todo bien"),
			...
			@ApiResponse(code = 403, message = "Prohibido el acceso"),
			@ApiResponse(code = 404, message = "No encontrado") })
	@ApiOperation(value = "Busca una persona por id del tipo de docuemnto y número de documento", notes = "Porpocionar un id del tipo de docuemnto y número de documento para encontrar la persona a la que corresponde", response = PersonaDTO.class)
	public ResponseEntity<PersonaDTO> BuscarPersona(
			@ApiParam(value = "Id del Tipo de documento", required = true) @PathVariable("idTipoDeDocumento") int idTipoDeDocumento,
			@ApiParam(value = "Número de documento", required = true) @PathVariable("numeroDeDocumento") String numeroDeDocumento) {
	...
	public ResponseEntity<List<PersonaDTO>> BuscarPersonaa(
			@ApiParam(value = "Edad para la condición", required = true) @PathVariable("edad") int edad) {
	...
	@ApiResponses(value = { @ApiResponse(code = 200, message = "Todo bien"),
			...
			@ApiResponse(code = 403, message = "Prohibido el acceso"),
			@ApiResponse(code = 404, message = "No encontrado") })
	@ApiOperation(value = "Actualizar una persona", notes = "Porpocionar el identificador(id) y demás datos necesarios para actualizar una persona")
